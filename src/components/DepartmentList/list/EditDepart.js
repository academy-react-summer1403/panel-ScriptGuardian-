...
import {
  useAddDepartment,
  useUpdateDepartment,
} from "../../../core/services/api/Admin/Departmenthandel";
const EditDepart = ({ setShow, show, data }) => {
  //get builds
  const { data: Builds } = useGetBuildingList();

  const options =
    Builds &&
    Builds.filter((item) => item?.active).map((item) => ({
      value: item?.id,
      label: item?.buildingName,
    }));

  //
  const { mutate: UpdateProfile } = useUpdateDepartment();
